...
/* ─── Input field ────────────────────────────────────────────────── */
function Field({ icon: Icon, label, type = 'text', value, onChange, right, required }) {
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    // If user ends up here without a recovery session fragment, they shouldn't be here.
    // Supabase handles extracting the session from URL implicitly, but if no user is found 
    // it means the link was invalid, expired, or opened without the hash.
    ...
    // Slight delay to ensure Supabase parsed the URL hash
    setTimeout(() => {
      checkSession();
    }, 500);
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Sign them out locally so they have to properly login through our Redux flow
          ...
          >
            ...
            <div className="text-center mb-6">
              <h1 className="text-[28px] font-black mb-1.5" style={{ fontFamily: 'Outfit, sans-serif', color: PRIMARY }}>
                Reset <span style={{ color: GOLD }}>Password</span>
              </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="relative w-full h-12 mt-2 rounded-xl font-bold text-[15px] text-white overflow-hidden"
                style={{
              ...
              >
                ...
                <AnimatePresence mode="wait">
                  {loading ? (
                    ...
                    <motion.span key="lbl" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}>Update Password</motion.span>
                  )}
                </AnimatePresence>
              </motion.button>
            </form>
